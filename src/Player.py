...
class Player():
    ...
    def declareBankruptcy(self, playersList, bank): 
        #Get the list of debts and title deeds
        debtItems = self.getDebtRecord()
        titleDeedRecords = self.getTitleDeeds() 
        ...
        for titleDeed in titleDeedRecords: 
            ...
            if (titleDeed["Title Deed"].getTitleType() == "Property"): 
                ...
                if (titleDeed["Hotels"] > 0):
                    ...
                    self.sellHotel(propertyName, sellHotelAmount, bank)
                ...
                if (titleDeed["Houses"] > 0):
                    ...
                    for i in range(numberHouses):
                        ...
                        self.sellHome(propertyName, sellHouseAmount, bank)
                        monetaryBuildingValue += sellHouseAmount
        
        self.changeMonetaryValue(monetaryBuildingValue)
        ...
        for debtAmount in debtAmounts: 
            ...
            #Search for that player in player list, and pay debts
            for indebitedPlayer in self.players: 
                if (indebitedPlayer.getName() == repayPlayerName): 
                    #If player has enough funds to clear debt
                    if (self.getMonetaryValue() > 0): 
